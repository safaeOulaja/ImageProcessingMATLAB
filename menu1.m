...
if length(size(im)) == 3 %Dimension de l'image est 3
...
for i = (rayon + 1):n - rayon
    for j = (rayon + 1):m - rayon
        ...
        intensite_cental = image_courant_mask(rayon + 1, rayon + 1);
        s = exp(-1 * (((image_courant_mask - intensite_cental) / max_reponse).^6));
        somme = sum(s(:));
        %   si le centre du mask est un 0 il faut soustraire les zeros des filtres
        if (intensite_cental == 0)
...
% Charger une image en niveaux de gris
    ...
    if size(image, 3) == 3 %nombre de caneaux
...
function FPBB_Callback(hObject, eventdata, handles)
 % R?cup?rer l'image
    ...
    % D?finir les param?tres du filtre Butterworth passe-bas
    D0 = 30; % Fr?quence de coupure
    n = 3;   % Ordre du filtre

    % Cr?er le filtre Butterworth passe-bas
    H = zeros(M, N);
    for i = 1:M
        for j = 1:N
    ...
    % Appliquer le filtre ? la transform?e de Fourier de l'image
    G = F .* H;

    % Effectuer la transform?e de Fourier inverse pour obtenir l'image filtr?e
    g = ifft2(ifftshift(G));
